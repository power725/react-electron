...
function AddProductionBlock(props) {
                    ...
                    <div className={`absolute top-0 right-0 block bg-white shadow-xl transform transition-all h-full p-3 w-96 flex flex-col justify-between`}>
                        <div>
                            <div className="px-2 pb-1 border-b-2 border-gray-normal my-2">
                                <h5 className="text-lg font-semibold text-gray-dark">İş İstasyonu Ekle</h5>
                            </div>

                            <div className="border rounded-lg border-gray-normal bg-gray-lightest p-2 pb-1">
                                <FormGroup label="İstaston Kodu">
                        ...
                        {isAddProductionModalOpen &&
                            <AddProductionModal onClose={() => setAddProductionModalOpen(false)} />
                        }
                    </div>
                </div>
            </div>
...
AddProductionBlock.defaultProps = {
    onClose: () => {}
}

AddProductionBlock.propTypes = {
    onClose: PropTypes.func
}

export default (props) => ReactDOM.createPortal(<AddProductionBlock {...props} />, document.getElementById('modal-root'));